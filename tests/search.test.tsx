...
test("normalizes aliases and diacritics", () => { assert.equal(normalizeSearch("Québec!") , "quebec"); assert.ok(searchPlaces("quebec illustrative municipalite").length); });
...
test("renders alias results with locale-correct links and keeps Explore in header navigation", () => {
  assert.match(renderToStaticMarkup(<SearchPage locale="en" query="quebec illustrative municipalite" />), /href="\/en\/places\//);
  assert.match(renderToStaticMarkup(<SearchPage locale="fr" query="quebec illustrative municipalite" />), /href="\/fr\/lieux\//);
