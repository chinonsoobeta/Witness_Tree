...
test("Phase 6 records exactly the five plan exit criteria and its honest local result", async () => {
  ...
  assert.equal(record.percentage, 80);
  assert.equal(record.phaseComplete, false);
  assert.deepEqual(record.exitCriteria.filter((item) => item.status === "fail").map((item) => item.id), ["kill-switch-under-five-minutes"]);
});
...
test("Phase 6 rejects an invented completion, altered percentage, missing blocker, or tampered evidence", async () => {
  ...
  await assert.rejects(validatePhase6AccountAlertExitStatus({ ...record, percentage: 60 }), /unweighted/);
