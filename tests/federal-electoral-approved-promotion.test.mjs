...
function liveIam() {
  return {
    ...
    roleArn: "arn:aws:iam::286853118812:role/WitnessTreeFederalElectoralPromotionUploader",
    roleIdentityArn: "arn:aws:sts::286853118812:assumed-role/WitnessTreeFederalElectoralPromotionUploader/witness-tree-federal-electoral-promotion",
...
case "$1:$2" in
  ...
  sts:get-caller-identity)
    ...
    else print -- '{"UserId":"assumed-role","Account":"286853118812","Arn":"arn:aws:sts::286853118812:assumed-role/WitnessTreeFederalElectoralPromotionUploader/witness-tree-federal-electoral-promotion"}'; fi ;;
...
function invoke(dir, phase = "success", extraEnv = {}) {
  const privateOutput = join(dir, "private.json"); const publicOutput = join(dir, "public.json"); const livePath = join(dir, "live-iam.json"); writeFileSync(livePath, `${JSON.stringify(liveIam(), null, 2)}\n`, { mode: 0o600 }); const marker = fakeAws(dir, phase);
...
expect {
  ...
  const run = spawnSync("expect", ["-c", program], { encoding: "utf8", timeout: 120_000, env: { ...process.env, ...extraEnv, PATH: `${dir}:${process.env.PATH}`, FEDERAL_DATA_ROOT: dataRoot, FEDERAL_IAM_ATTESTATION: livePath, FEDERAL_PRIVATE_OUTPUT: privateOutput, FEDERAL_PUBLIC_OUTPUT: publicOutput } });
  return { run, privateOutput, publicOutput, marker, livePath };
}
...
test("live IAM gate is required before MFA and does not leak provider values", () => {
  ...
  try {
    ...
    const run = spawnSync("zsh", [runner, "--run"], { encoding: "utf8", env: { ...process.env, PATH: `${dir}:${process.env.PATH}`, FEDERAL_DATA_ROOT: dataRoot, FEDERAL_PRIVATE_OUTPUT: join(dir, "private"), FEDERAL_PUBLIC_OUTPUT: join(dir, "public"), FEDERAL_IAM_ATTESTATION: join(dir, "missing-live.json") }});
...
test("successful run binds conditional versions, local/provider checksums, post-write version lists, IAM digests, and primary-only credit boundary", () => {
  ...
  try {
    const { run, marker, privateOutput, publicOutput } = invoke(dir); assert.equal(run.status, 0, `${run.stdout}\n${run.stderr}`); const publicValue = JSON.parse(readFileSync(publicOutput)); validateFederalAttestationPair(privateOutput, publicValue, plan); validateRedactedFederalAttestation(publicValue, plan); assert.match(readFileSync(privateOutput, "utf8"), /rawResponses|approvalSha256|liveIamSha256|recoveryCreditEligible/); assert.doesNotMatch(readFileSync(publicOutput, "utf8"), /payload-version-1|manifest-version-1|arn:aws:iam/);
    const calls = readFileSync(marker, "utf8"); assert.match(calls, /head-object .*--if-none-match \*/); assert.match(calls, /put-object .*--if-none-match \*/); assert.match(calls, /put-object .*--checksum-algorithm SHA256 .*--checksum-sha256/); assert.match(calls, /head-object .*--version-id payload-abc12345/); assert.match(calls, /head-object .*--version-id manifest-abc12345/); assert.match(calls, /list-object-versions/); assert.doesNotMatch(calls, /delete|abort|bypass/i); assert.equal(publicValue.recoveryBoundary.recoveryCreditEligible, false); assert.equal(publicValue.claims.sourceLedgerCreditChanged, false);
  } finally { rmSync(dir, { recursive: true, force: true }); }
});
