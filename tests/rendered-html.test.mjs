...
test("renders localized search results and Explore list/table alternatives without browser JavaScript", async () => {
  ...
  assert.match(englishExplore, /Illustrative fixtures only — no live data or geometry is shown\./);
  ...
  assert.match(frenchExplore, /Exemples illustratifs seulement — aucune donnée ni géométrie en direct n’est affichée\./);
