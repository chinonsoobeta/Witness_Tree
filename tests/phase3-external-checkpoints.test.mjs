...
const scopes = (templates, locales, modes = [null]) => templates.flatMap((template) => locales.flatMap((locale) => modes.map((mode) => ({ template, locale, ...(mode ? { mode } : {}), evidenceOrigin: "human-manual", automationGenerated: false, testerAttestationReference: `attestation-${template}-${locale}-${mode ?? "default"}`, startedAt: "2026-09-20T16:00:00.000Z", endedAt: "2026-09-20T16:30:00.000Z", passed: true, blockedTasks: 0, issueIds: [], environment: "test-only human-evidence fixture" }))));
...
function completeEvidence() {
  ...
  return {
    schemaVersion: "witness-tree/phase3-external-checkpoint-evidence/1",
    ...
    manualAccessibility: {
      ...
      screenReader: scopes(SCREEN_READER_TEMPLATES, LOCALES),
      forcedColorsAndCvd: scopes(TEMPLATES, LOCALES, CVD_MODES),
      issues: []
    },
    fieldPerformance: LOCALES.map((locale) => ({ locale, metric: "LCP", statistic: "p75", valueMs: 1500, eligibleSamples: 100, windowStartedAt: "2026-09-01T00:00:00.000Z", windowEndedAt: "2026-09-29T00:00:00.000Z", aggregateReportReference: `test-only-field-report-${locale}`, containsParticipantIdentifiers: false })),
