...
test("proposed derived IAM is MFA-only and limited to four exact keys with version-specific recovery readback", () => { const desired=validateIamDesiredState(); const [objects,retention]=desired.rolePolicy.Statement; assert.deepEqual(objects.Action,["s3:PutObject","s3:GetObject","s3:GetObjectVersion"]); assert.equal(objects.Resource.length,4); assert.equal(retention.Resource.length,2); assert.ok([...objects.Resource,...retention.Resource].every((key)=>key.includes("/derived/") && !key.includes("*"))); });
