...
const args = [read("data/phase1-immutable-promotion-readiness.json"), read("data/phase1-production-source-ledger.json"), read("data/phase1-local-profiled-promotion-preparation.json"), read("data/current-wildfire-immutable-promotion-preparation.json"), read("data/current-wildfire-owner-admission.json"), read("data/qc-immutable-promotion-preparation.json"), read("data/qc-fourth-inventory-evidence.json"), read("data/qc-fourth-inventory-immutable-promotion-preparation.json"), read("data/qc-fourth-inventory-immutable-promotion-iam-policy.json")];
...
test("immutable-promotion readiness accounts for every local-profiled row without duplicate work", () => {
  ...
  assert.equal(args[0].coveredProductionRowIds.length, 5);
...
test("the fourth-inventory archive is exact-key prepared but stays fail-closed without separate approvals or remote evidence", () => {
  ...
  assert.throws(() => validatePhase1ImmutablePromotionReadiness(args[0], ...args.slice(1, 7), remote, args[8]));
});
