...
test("Phase 8 records every literal launch-readiness gate without production inflation", async () => {
  ...
  assert.equal(record.completedCriteria, 5);
  assert.equal(record.totalCriteria, 16);
  assert.equal(record.percentage, 31.25);
  assert.equal(record.phaseComplete, false);
  assert.deepEqual(record.exitCriteria.filter((item) => item.status === "pass").map((item) => item.id), ["governance-and-corrections-procedures", "operations-handbook", "citation-format", "release-notes", "restore-tests"]);
});
