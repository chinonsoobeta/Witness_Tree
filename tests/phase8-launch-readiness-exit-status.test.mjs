...
test("Phase 8 records every literal launch-readiness gate without production inflation", async () => {
  ...
  assert.deepEqual(record.exitCriteria.filter((item) => item.status === "pass").map((item) => item.id), ["governance-and-corrections-procedures", "citation-format", "release-notes", "restore-tests"]);
});
