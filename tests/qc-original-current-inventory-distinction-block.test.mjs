...
test("QC original/current inventory rejects a current-map substitution or incomplete-acquisition claim", () => {
  ...
  for (const altered of [{ ...record, distinction: { ...record.distinction, sameArtifact: true } }, { ...record, authorityAndAccess: { ...record.authorityAndAccess, exactArtifactProfileVerified: true } }, { ...record, actions: { ...record.actions, reusedCurrentArchive: true } }, { ...record, actions: { ...record.actions, untrustedConcurrentPartialQuarantined: false } }, { ...record, actions: { ...record.actions, downloadComplete: true } }]) assert.throws(() => validateQcOriginalCurrentInventoryDistinctionBlock(altered));
});
