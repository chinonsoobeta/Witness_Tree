...
test("a successful retention request without a COMPLIANCE read-back fails closed", () => {
  ...
  const invoke = (args: string[]) => {
    ...
    const payload = plan.entries.find((entry: any) => entry.remote.payloadKey === key);
    ...
    if (operation === "head-object") return { ContentLength: Buffer.byteLength(sidecarFor(plan, plan.entries.find((entry: any) => entry.remote.manifestKey === key))), VersionId: "x".repeat(32), ChecksumType: "FULL_OBJECT", ChecksumCRC64NVME: "AAAAAAAAAAA=" };
