...
const run = (events: readonly AlertEvent[], history: readonly { areaId: string; eventId: string; fingerprint: string; sentAt: string; dataVersion: string; cadence: "immediate"; releaseId?: string }[] = []) => evaluateAlerts({ account, areas: [area], events, history, now: "2026-08-11T12:00:00Z" });
...
test("all seven triggers have prescribed defaults and complete bilingual payloads", () => {
  ...
  const french = evaluateAlerts({ account, areas: [{ ...area, alertLocale: "fr" }], events: [event("official-record")], history: [], now: "2026-08-11T12:00:00Z" }).alerts[0]!;
...
test("unverified and unsubscribed accounts never produce alert payloads", () => {
  assert.equal(evaluateAlerts({ account: { ...account, emailVerifiedAt: undefined }, areas: [area], events: [event("official-record")], history: [], now: "2026-08-11T12:00:00Z" }).alerts.length, 0);
  assert.equal(evaluateAlerts({ account: { ...account, unsubscribedAt: "2026-08-11T11:00:00Z" }, areas: [area], events: [event("official-record")], history: [], now: "2026-08-11T12:00:00Z" }).alerts.length, 0);
});
