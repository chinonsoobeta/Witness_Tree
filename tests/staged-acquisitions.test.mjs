...
const albertaWildfire = manifest.entries.find((entry) => entry.sourceId === "ab-wildfire");
...
test("verified local acquisition remains staging-only", () => {
  ...
  assert.equal(manifest.entries.reduce((total, entry) => total + entry.byteLength, 0), 10955964208);
  assert.equal(alberta?.sha256, "e93572129f25c83911b73eadfacff12624ff6b08f2db4b311c1662196b665093");
});

test("staged Alberta current wildfire locations are checksum-bound and remain staging-only", () => {
  assert.ok(albertaWildfire, "ab-wildfire entry is missing from the manifest");
  assert.equal(albertaWildfire.byteLength, 423853);
  assert.equal(albertaWildfire.sha256, "f0e86ea34a7624c365349b3a8fbb77967bb45ab73c507cf441efb8f6a8736ee0");
  assert.equal(albertaWildfire.contentIntegrity, "passed");
  assert.match(albertaWildfire.sourceUrl, /alberta_fire_status/);
