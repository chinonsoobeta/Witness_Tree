...
import { commandPlan, executeArchiveControls, validateArchiveControlsExecution, validateExecutionOptions } from "../scripts/phase1-archive-controls-execution.mjs";

const plan = JSON.parse(readFileSync(new URL("../data/phase1-archive-controls-execution.json", import.meta.url), "utf8"));
const approved = (workDir) => ({ execute: true, "approve-identities": true, "approve-audit-logging": true, "approve-inventory": true, "approve-lifecycle": true, "approve-legal-hold-exercise": true, "approve-recovery-replication": true, "approve-usd-10-monthly-ceiling": true, accountId: "123456789012", ownerFederatedPrincipalArn: "arn:aws:iam::123456789012:role/OwnerFederatedArchiveOperator", exerciseRetentionUntil: "2033-08-12T00:00:00Z", exerciseKey: "raw/legal-hold-exercises/2030-01-01/test-run/payload.txt", exercisePayloadFile: join(workDir, "payload.txt"), monthlyCeiling: 10, workDir });
...
test("execution fails closed without every approval, a Canadian account principal, bounded cost, or dedicated exercise key", () => {
  ...
  try {
    ...
    assert.throws(() => validateExecutionOptions(plan, { ...options, ownerFederatedPrincipalArn: "arn:aws:iam::999999999999:role/wrong" }), /supplied account/);
