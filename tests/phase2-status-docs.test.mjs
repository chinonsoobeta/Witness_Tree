...
const componentInventory=await readFile(new URL("../docs/PHASE2_REAL_LOSS_COMPONENT_INVENTORY.md",import.meta.url),"utf8");
const phase2Row=matrix.split("\n").find(line=>line.startsWith("| **2. National baseline pipeline (1984–2022)**"));

test("Phase 2 status documents pin the independently audited 54-point allocation",()=>{
  ...
  assert.match(componentInventory,/fixed-rubric score of \*\*54% \(\+3 percentage points from 51%\)\*\*/);
...
test("the current Phase 2 matrix records the real raster batch and every retained gap",()=>{
  ...
  for(const gap of ["Vectorize the real component lineage into patches and normalized events","all eight versioned boundary intersections and aggregates","independent validation samples and statistics","generate tiles/downloads","complete release and exit evidence"])assert.match(phase2Row,new RegExp(gap));
  assert.match(phase2Row,/exact component lineage.*303,530,909 four-connected components/);
  assert.match(phase2Row,/\| \*\*54%\*\* \|$/);
  assert.doesNotMatch(phase2Row,/illustrative annual fixtures|Fetch and process NTEMS|create annual masks|\*\*15%\*\*/);
  assert.match(status,/accepted this evidence at \*\*54% \(\+3 percentage points from 51%\)\*\*/);
});
