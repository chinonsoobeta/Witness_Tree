...
type Phase2Aggregate = Readonly<{
  ...
  shareOfFirstYearForest: Readonly<{ kind: "figure"; percent: number }> | Readonly<{ kind: "unknown"; reason: LocalizedString }>;
...
const localizedReason = (reason: LocalizedString): LocalizedString => ({ en: reason.en, fr: reason.fr });
...
export function adaptPhase2SyntheticAggregates(document: Phase2SyntheticAggregateDocument, retrievedDate: string): readonly AdaptedPhase2Aggregate[] {
  ...
  return Object.freeze(document.aggregates.map((aggregate) => {
    ...
    if (!Number.isSafeInteger(aggregate.timeRange.fromYear) || !Number.isSafeInteger(aggregate.timeRange.toYear) || aggregate.timeRange.fromYear > aggregate.timeRange.toYear || aggregate.denominator.kind !== "forested-hectares" || !Number.isSafeInteger(aggregate.denominator.referenceYear) || aggregate.denominator.referenceYear !== aggregate.timeRange.fromYear) throw new Error("Phase 2 synthetic aggregate years and denominator reference year are invalid.");
    if (aggregate.denominator.hectares < 0 || aggregate.eventHectares < 0) throw new Error("Phase 2 denominator and event hectares must be nonnegative.");
    if (aggregate.shareOfFirstYearForest.kind === "figure") {
      finite(aggregate.shareOfFirstYearForest.percent, "share percent");
      const expected = aggregate.denominator.hectares === 0 ? Number.NaN : aggregate.eventHectares / aggregate.denominator.hectares * 100;
      if (aggregate.shareOfFirstYearForest.percent < 0 || aggregate.shareOfFirstYearForest.percent > 100 || !Number.isFinite(expected) || Math.abs(aggregate.shareOfFirstYearForest.percent - expected) > 1e-9) throw new Error("Phase 2 share must be 0..100 and consistent with event and denominator hectares.");
    } else if (!aggregate.shareOfFirstYearForest.reason.en.trim() || !aggregate.shareOfFirstYearForest.reason.fr.trim()) {
      throw new Error("Phase 2 Unknown share requires nonblank English and French reasons.");
    }
    ...
    const share: PublicNumber = aggregate.shareOfFirstYearForest.kind === "figure"
      ...
      : { kind: "unknown", evidence: "unknown", reason: localizedReason(aggregate.shareOfFirstYearForest.reason), coverageGrade: "national-baseline", provenance };
    return Object.freeze({
      ...
      forestDefinitionVersion: aggregate.denominator.forestDefinitionVersion, methodVersion: aggregate.methodVersion, dataVersion: aggregate.dataVersion, lineageSha256: aggregate.lineageSha256,
    });
  }));
}
