...
/** Produces aggregate-only quarterly metrics; it retains and reveals no case-level data. */
export function correctionMetrics(cases: readonly CorrectionCase[]): CorrectionMetricsResult {
  ...
  if (resolved.some((caseFile) => !caseFile.resolvedAt)) throw new Error("Resolved corrections require a resolution timestamp.");
