...
export function AnnualChangeChart({ annual, locale, view }: Readonly<{ annual: readonly AnnualSummary[]; locale: Locale; view: "chart" | "table" }>) {
  ...
  if (view === "table") return <section><h2>{title}</h2><table><thead><tr><th>{locale === "en" ? "Year" : "Année"}</th><th>{locale === "en" ? "Hectares" : "Hectares"}</th><th>{locale === "en" ? "Event IDs" : "Identifiants d’événement"}</th></tr></thead><tbody>{rows.map((row) => <tr key={row.year}><td>{row.year}</td><td>{row.hectares}</td><td>{row.eventIds.join(", ")}</td></tr>)}</tbody></table></section>;
