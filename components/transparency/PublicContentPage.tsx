...
export function PublicContentPage({ kind, locale }: Readonly<{ kind: PublicContentKind; locale: Locale }>) {
  ...
  return <main id="main" tabIndex={-1} className="page-wrap"><header className="masthead"><p>{locale === "en" ? "Illustrative content · unapproved · nonproduction" : "Contenu illustratif · non approuvé · hors production"}</p><h1>{page.title[locale]}</h1><p className="dek">{page.status[locale]}</p></header><div className="content-section prose-measure">
