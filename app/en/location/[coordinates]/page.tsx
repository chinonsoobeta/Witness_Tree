...
export async function generateMetadata({ params }: { params: Promise<{ coordinates: string }> }): Promise<Metadata> {
  ...
  const record = localizedRecord("location", coordinates, "en");
...
export default async function EnglishLocationPage({ params }: Readonly<{ params: Promise<{ coordinates: string }> }>) {
  const { coordinates } = await params;
  ...
  return <SiteShell locale="en"><LocationResult locale="en" location={entry.location} places={places} /></SiteShell>;
}
