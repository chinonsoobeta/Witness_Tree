...
function coverageLabel(row: (typeof EXPLORE_PRODUCTION_LAYER.rows)[number]) {
  ...
  return `Minimum from the mapped area; ${number.format(row.unknownSharePercent)}% (${number.format(row.unknownRequiredInputHectares)} ha) is unknown`;
}

export default function EnglishHome() {
  return <SiteShell locale="en"><main id="main" className="page-wrap">
    <header className="masthead">
      <p className="eyebrow">Evidence record · 1984 to 2022</p>
    ...
    <section className="content-section prose-measure" aria-labelledby="current-record">
      ...
      <p className="lead">The bounded, provisional {EXPLORE_PRODUCTION_LAYER.period} province aggregate is available to explore. It reports detected forest loss with a coverage state for each province. A complete 1984 to 2022 mapped-extent verification now governs how unknown areas are classified.</p>
      ...
      <p><a className="btn btn--primary" href="/en/explore">Explore the province aggregate</a></p>
    ...
    <section className="content-section">
      ...
      <div className="record-grid">
        ...
        <article className="record-card"><p className="eyebrow">Data status</p><h3>Bounded province release</h3><p>The 2020 to 2022 province aggregate is published with its source, coverage state and limits. Other views may still use clearly labelled examples.</p><a href="/en/data">Review data transparency</a></article>
      </div>
    ...
    <section className="content-section prose-measure" aria-labelledby="consequences">
      ...
      <p>Observed forest loss is a satellite-derived measure, not a finding about cause. A detected reduction in tree cover does not by itself establish logging, deforestation, responsibility or compliance. <a href="/en/methods">Read the method and evidence definitions</a>.</p>
      <p>The available release is a deterministic, four-province province-level technical preview for 2020 to 2022. It is not per-cell geometry and does not complete the formal Phase 2 gate. <a href="/en/data">Read the release scope, provenance and licence attribution</a>.</p>
