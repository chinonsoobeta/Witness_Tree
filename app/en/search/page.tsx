...
export const metadata: Metadata = { title: "Search places", alternates: localizedAlternates("en", { en: "/en/search", fr: "/fr/recherche" }) };
export default async function Page({ searchParams }: Readonly<{ searchParams: Promise<{ q?: string; district?: string; scope?: string }> }>) { const query = await searchParams;
  ...
  ]); const scope = query.scope === "districts" || (query.district !== undefined && query.q === undefined) ? "districts" : "places"; return <SiteShell locale="en"><main id="main"><SearchPage locale="en" addressLookup={addressConfigured && districtConfigured} scope={scope} query={scope === "districts" ? query.district ?? "" : query.q ?? ""} /></main></SiteShell>; }
