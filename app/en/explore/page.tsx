...
export default async function Page({ searchParams }: { searchParams: Promise<{ mode?: string; presentation?: string; data?: string }> }) {
  ...
  return <SiteShell locale="en"><main id="main" className="page-wrap"><h1>Explore forest change</h1>{presentation === "map" ? <ExploreMapClient events={events} locale="en" /> : null}<ExploreView events={exploreFixtures} locale="en" mode={mode} presentation={presentation} data={query.data === "table" ? "table" : "chart"} /></main></SiteShell>;
}
