...
export function validateVlce2ForestMaskDecision(doc) {
  ...
  if (failures.length > 0) {
    throw new Error(`${PATH} failed the forest-mask decision gate:\n${failures.map((failure) => `  - ${failure}`).join("\n")}`);
  }
  return { classCount: CLASS_CODES.length, permitted };
}

if (import.meta.url === `file://${process.argv[1]}`) {
  try {
    const result = validateVlce2ForestMaskDecision(readFileSync(PATH, "utf8"));
    console.log(`VLCE2 forest-mask decision gate passed: ${result.classCount} classes unresolved, mask implementation not permitted.`);
  } catch (error) {
