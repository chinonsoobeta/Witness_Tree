...
export function validateBcForestOperationsMapAccess(record) {
  ...
  if (!request || request.status !== "draft-not-sent" || request.proposedSender !== "Chinonso Obeta <chinonso8@gmail.com>" || request.recipientRoute !== "https://forms.gov.bc.ca/copyright-permission-request/" || request.copyrightContact !== "QPIPPCopyright@gov.bc.ca" || request.subject !== "Permission request — Forest Operations Map (FOM) - Cutblocks" || request.authorizationOutcome !== "not-requested") throw new Error("Only bounded, unsent permission-request evidence is allowed.");
