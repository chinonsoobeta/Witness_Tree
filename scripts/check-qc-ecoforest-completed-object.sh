...
creds="$(aws sts assume-role --role-arn "arn:aws:iam::${ACCOUNT}:role/${ROLE}" --role-session-name witness-tree-qc-completed-object-check --duration-seconds 3600 --output json)" || fail "Approved read-only role assumption failed" 77
