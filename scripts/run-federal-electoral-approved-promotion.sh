...
OWNER_PACKET="${FEDERAL_OWNER_PACKET:-$ROOT/data/phase1-owner-approval-packet.json}"
ARCHIVE_READINESS="${FEDERAL_ARCHIVE_READINESS:-$ROOT/data/archive-operations-readiness.json}"
...
gate_args=(--plan "$PLAN" --approval "$APPROVAL" --iam "$IAM_DESIRED" --owner-packet "$OWNER_PACKET" --readiness "$ARCHIVE_READINESS")
[[ "$1" == "--run" ]] && gate_args+=(--require-ready)
node "$ROOT/scripts/check-federal-electoral-promotion-gates.mjs" "${gate_args[@]}" >/dev/null || fail "Federal plan, owner packet, owner approval, IAM desired-state, or archive-readiness gate failed; no TOTP or AWS call was made" 75
...
node "$ROOT/scripts/check-federal-electoral-promotion-gates.mjs" --plan "$PLAN" --approval "$APPROVAL" --iam "$IAM_DESIRED" --owner-packet "$OWNER_PACKET" --readiness "$ARCHIVE_READINESS" --live-iam "$IAM_ATTESTATION" --require-live --require-ready >/dev/null || fail "Federal live IAM policy/actions/resources or archive-readiness gate failed; no TOTP or AWS call was made" 75
AUTHORIZATION_SHA256="$(jq -cn \
  --arg plan "$(sha256_hex "$PLAN")" \
  --arg approval "$(sha256_hex "$APPROVAL")" \
  --arg iamDesired "$(sha256_hex "$IAM_DESIRED")" \
  --arg liveIam "$(sha256_hex "$IAM_ATTESTATION")" \
  --arg ownerPacket "$(sha256_hex "$OWNER_PACKET")" \
  --arg readiness "$(sha256_hex "$ARCHIVE_READINESS")" \
  --arg runner "$(sha256_hex "$ROOT/scripts/run-federal-electoral-approved-promotion.sh")" \
  '{plan:$plan,approval:$approval,iamDesired:$iamDesired,liveIam:$liveIam,ownerPacket:$ownerPacket,readiness:$readiness,runner:$runner}')" || fail "Federal pre-action authorization inputs could not be SHA-256 bound; no TOTP or AWS call was made" 75
jq -e 'all(.[]; test("^[a-f0-9]{64}$"))' <<<"$AUTHORIZATION_SHA256" >/dev/null || fail "Federal pre-action authorization SHA-256 set is malformed; no TOTP or AWS call was made" 75
...
node "$ROOT/scripts/assemble-federal-electoral-promotion-attestation.mjs" --capture-dir "$CAPTURE_DIR" --plan "$PLAN" --approval "$APPROVAL" --iam "$IAM_DESIRED" --live-iam "$IAM_ATTESTATION" --owner-packet "$OWNER_PACKET" --readiness "$ARCHIVE_READINESS" --runner "$ROOT/scripts/run-federal-electoral-approved-promotion.sh" --authorization-sha256 "$AUTHORIZATION_SHA256" --private "$PRIVATE_OUTPUT" --public "$PUBLIC_OUTPUT" || fail "Federal attestation assembly failed; preserve owner-only capture" 70
