...
unset bootstrap
identity="$(aws sts get-caller-identity --region "$REGION" --output json)" || fail "cannot identify MFA session" 77
...
unset identity
role_session="$(aws sts assume-role --role-arn "$ROLE_ARN" --role-session-name "$SESSION_NAME" --duration-seconds 3600 --region "$REGION" --output json)" || fail "read-only role assumption failed" 77
...
unset role_session
role_identity="$(aws sts get-caller-identity --region "$REGION" --output json)" || fail "cannot identify the assumed read-only role" 77
