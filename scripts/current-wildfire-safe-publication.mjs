...
export function publishCurrentWildfireMode600(path, value, hooks = {}) {
  ...
  try {
    ...
    const written = fstatSync(fd); assert.equal(same(written, opened) && written.size === bytes.length && written.nlink === 1, true); const named = lstatSync(output); assert.equal(same(named, opened) && named.nlink === 1, true); fsyncSync(parentFd); closeSync(fd); fd = undefined;
