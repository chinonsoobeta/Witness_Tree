...
function validateBaseline(baseline, staged) {
  ...
  if (baseline.immutableObjectStorage !== false || baseline.productionEligible !== false) {
    throw new Error("A source-review baseline must never claim immutability or production eligibility.");
  }
...
export function validateSourceReviewRegister(register, staged) {
  ...
  for (const baseline of register.baselines) {
    validateBaseline(baseline, staged);
...
export async function checkSourceReview(registerFile, stagedFile) {
  const [register, staged] = await Promise.all([
  ...
  return validateSourceReviewRegister(JSON.parse(register), JSON.parse(staged));
}

if (import.meta.url === `file://${process.argv[1]}`) {
  const directory = path.dirname(fileURLToPath(import.meta.url));
  const register = await checkSourceReview(path.resolve(directory, "../data/source-review-register.json"), path.resolve(directory, "../data/staged-acquisitions.json"));
