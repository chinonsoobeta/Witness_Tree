...
function validateIam(iam, keys) {
  ...
  assert.deepEqual(
    ...
    new Set(["ObtainMfaSession", "ExactMfaGatedPromotionObjectsOnly", "DenyObjectAccessOutsideExactObjectSet", "DenyUnscopedBucketListing", "DenyDeletionBypassAndBucketMutation"]),
  );
  assert.equal(iam.Statement.length, 5);
  const session = iam.Statement.find((statement) => statement.Sid === "ObtainMfaSession");
  assert.deepEqual(session, { Sid: "ObtainMfaSession", Effect: "Allow", Action: "sts:GetSessionToken", Resource: "*" });
