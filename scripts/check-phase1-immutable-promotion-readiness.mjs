...
export function validatePhase1ImmutablePromotionReadiness(audit, ledger, national, wildfire, quebec, fourth) {
  ...
  assert.equal(wildfireGroup.status, "already-prepared"); assert.equal(wildfireGroup.physicalArtifactCount, wildfire.artifacts.length); assert.equal(wildfireGroup.proposedRole, wildfire.mfaGatedExecution.proposedRole); assert.deepEqual([...wildfireGroup.productionRowIds].sort(), wildfire.artifacts.map((artifact) => read("data/staged-acquisitions.json").entries.find((entry) => entry.id === artifact.id).sourceId).sort());
  ...
  assert.equal(fourthGroup.status, "unprepared-fail-closed"); assert.equal(fourthGroup.physicalArtifactCount, fourth.fullProductAcquisition.archiveCount); assert.match(fourthGroup.blocker, /individual local paths, byte lengths, SHA-256 values, deterministic payload keys, and sidecars/i);
  for (const field of ["immutableObjectStorage", "transformed", "ingested", "productionAdmission", "productionEligible"]) assert.equal(fourth[field], false);
  return audit;
}

export function checkPhase1ImmutablePromotionReadiness() {
  return validatePhase1ImmutablePromotionReadiness(read("data/phase1-immutable-promotion-readiness.json"), read("data/phase1-production-source-ledger.json"), read("data/phase1-local-profiled-promotion-preparation.json"), read("data/current-wildfire-immutable-promotion-preparation.json"), read("data/qc-immutable-promotion-preparation.json"), read("data/qc-fourth-inventory-evidence.json"));
}
...
if (process.argv[1]?.endsWith("check-phase1-immutable-promotion-readiness.mjs")) {
  ...
  console.log(`Phase 1 immutable-promotion readiness audit passed: ${audit.coveredProductionRowIds.length} local-profiled rows, ${audit.physicalArtifactGroups.slice(0, 3).reduce((sum, group) => sum + group.physicalArtifactCount, 0)} already prepared physical artifacts, one 56-sheet fail-closed gap.`);
}
