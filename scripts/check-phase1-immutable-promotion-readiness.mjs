...
export function validatePhase1ImmutablePromotionReadiness(audit, ledger, national, wildfire, quebec, fourthEvidence, fourthPlan, fourthIam) {
...
export function checkPhase1ImmutablePromotionReadiness() {
  return validatePhase1ImmutablePromotionReadiness(read("data/phase1-immutable-promotion-readiness.json"), read("data/phase1-production-source-ledger.json"), read("data/phase1-local-profiled-promotion-preparation.json"), read("data/current-wildfire-immutable-promotion-preparation.json"), read("data/qc-immutable-promotion-preparation.json"), read("data/qc-fourth-inventory-evidence.json"), read("data/qc-fourth-inventory-immutable-promotion-preparation.json"), read("data/qc-fourth-inventory-immutable-promotion-iam-policy.json"));
}
...
if (process.argv[1]?.endsWith("check-phase1-immutable-promotion-readiness.mjs")) {
  ...
  console.log(`Phase 1 immutable-promotion readiness audit passed: ${audit.coveredProductionRowIds.length} local-profiled rows, ${audit.physicalArtifactGroups.reduce((sum, group) => sum + group.physicalArtifactCount, 0)} exact prepared objects; QC fourth remains execution-blocked pending separate approvals.`);
}
