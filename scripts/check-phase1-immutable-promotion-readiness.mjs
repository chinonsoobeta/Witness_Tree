...
export function validatePhase1ImmutablePromotionReadiness(audit, ledger, national, wildfire, wildfireAdmission, quebec, fourthEvidence, fourthPlan, fourthIam) {
  ...
  const rows = audit.physicalArtifactGroups.filter(({ id }) => id !== "current-wildfire-six-release-inputs").flatMap((group) => group.productionRowIds);
  ...
  assert.equal(wildfireGroup.status, "four-raw-objects-remotely-verified-two-derived-objects-missing");
  ...
  assert.equal(wildfireAdmission.archiveGate.verifiedObjectCount, 4);
  assert.match(wildfireGroup.blocker, /Four exact raw snapshots have (?:redacted version-present|matching redacted primary\/recovery version-present).*BC and Ontario derived releases remain unverified.*4\/6/i);
  ...
  assert.equal(fourthGroup.status, "already-prepared-blocked-pending-separate-approvals"); assert.equal(fourthGroup.preparation, "data/qc-fourth-inventory-immutable-promotion-preparation.json"); assert.equal(fourthGroup.iamDesiredState, "data/qc-fourth-inventory-immutable-promotion-iam-desired-state.json"); assert.equal(fourthGroup.proposedRole, "WitnessTreeQcFourthArchivePromotionUploader"); assert.equal(fourthGroup.runner, "node scripts/qc-fourth-inventory-immutable-promotion.mjs"); assert.match(fourthGroup.blocker, /exact 62-key plan.*non-applied exact-role IAM desired state.*no AWS or IAM call.*no remote object or read-back/i);
