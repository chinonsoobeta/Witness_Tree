...
import { validateRealExecutionEvidence } from "./check-phase2-real-national-execution-evidence.mjs";
...
validateRealExecutionEvidence(evidence,lineage);
...
assert.equal(evidence.schemaVersion,"witness-tree/phase2-real-national-execution-evidence/1"); assert.equal(evidence.batchId,lineage.batchId); assert.equal(evidence.reviewStatus,lineage.reviewStatus);
...
assert.equal(evidence.lineageSha256,await sha(lineageFile)); assert.equal(evidence.preflightSha256,lineage.preflight.sha256); assert.equal(evidence.executionEvidenceCoreSha256,lineage.execution.executionEvidenceCoreSha256); assert.equal(evidence.inputSetSha256,lineage.sourceVerification.inputSetSha256);
assert.ok(evidence.execution.elapsedSeconds > 0 && evidence.execution.elapsedSeconds < 96*3600); assert.ok(evidence.execution.requestedConcurrency <= 8 && evidence.execution.observedConcurrencyUpperBound <= 8); assert.equal(evidence.execution.ramCapBytes,16*1024**3); assert.ok(evidence.execution.diskBoundBytes <= evidence.execution.diskCapBytes && evidence.execution.diskCapBytes <= 2*1024**4);
