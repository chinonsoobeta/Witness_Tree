...
export function validatePhase1NationalArchiveFinalizationAudit(audit = read("data/phase1-national-archive-finalization-audit.json")) {
  ...
  assert.match(audit.notice, /Redacted.*offline-mock.*read-only.*No AWS.*TOTP.*production action/i);
  ...
  assert.deepEqual({ openUploadCount: canopy.openUploadCount, partCount: canopy.partCount, isTruncated: canopy.isTruncated, allPartsHaveProviderChecksum: canopy.allPartsHaveProviderChecksum }, { openUploadCount: 1, partCount: 155, isTruncated: false, allPartsHaveProviderChecksum: true });
  ...
  assert.deepEqual(audit.liveReadOnly.exactHeads, { canopy: { payloadExists: false, manifestExists: false }, federal: { payloadExists: false, manifestExists: false } });
...
if (process.argv[1]?.endsWith("check-phase1-national-archive-finalization-audit.mjs")) {
  ...
  console.log("Phase 1 national archive finalization audit passed: canopy has 155 provider-checksummed parts, offline completion is fixed, and no owner-run command is authorized by this evidence.");
}
