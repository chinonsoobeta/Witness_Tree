...
if ! aws sts get-session-token --serial-number "$mfa_serial" --token-code "$totp" --profile "$PROFILE" --duration-seconds 3600 --output json >"$TMP/bootstrap.json" 2>"$TMP/sts-session.stderr"; then
  fail "MFA session failed; no storage mutation was authorized" 77
fi
...
account="$(aws sts get-caller-identity --query Account --output text 2>"$TMP/caller.stderr")" || fail "MFA session identity could not be verified; no storage mutation was authorized" 77
[[ "$account" == "$ACCOUNT" ]] || fail "MFA session is outside the approved account; no storage mutation was authorized" 77
if ! aws sts assume-role --role-arn "arn:aws:iam::$ACCOUNT:role/$ROLE" --role-session-name witness-tree-canopy-recovery --duration-seconds 3600 --output json >"$TMP/role-session.json" 2>"$TMP/sts-role.stderr"; then
