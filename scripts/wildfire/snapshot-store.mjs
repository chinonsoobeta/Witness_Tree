...
function sourceStatus({ prior = {}, source, timestamp, snapshot, now }) {
  if (source.response != null) {
    return {
      id: source.id, status: 'healthy', stale: false, consecutiveFailures: 0,
      lastSuccessAt: timestamp, lastGoodSnapshot: snapshot,
    };
  }
  ...
  return {
    ...
    stale: Boolean(prior.lastSuccessAt && now.getTime() - new Date(prior.lastSuccessAt).getTime() > 24 * 60 * 60 * 1000),
    consecutiveFailures,
    ...
    async publish({ sources, now = new Date() }) {
      ...
      for (const source of sources) {
        ...
        if (source.response != null) {
          ...
          snapshotPath = file('snapshots', `${version}-${safeName(source.id)}.json`);
