...
const READ_ONLY_OPERATIONS = new Set(["s3api head-object", "s3api get-object", "s3api head-bucket", "s3api list-object-versions", "s3api get-object-attributes", "sts get-caller-identity"]);
...
export function validateRawArchiveReproductionDrill(record, { archiveEvidence, admission, runnerSha256 = runnerSha256OnDisk() } = {}) {
  ...
  checkArchivedObject(restored.payload, { key: archiveEvidence.physicalArtifact?.payloadKey, versionId: archiveEvidence.payload?.versionId, byteLength: archiveEvidence.payload?.byteLength, sha256: archiveEvidence.payload?.sha256 }, "payload");
  checkArchivedObject(restored.manifest, { key: archiveEvidence.physicalArtifact?.manifestKey, versionId: archiveEvidence.manifest?.versionId, byteLength: archiveEvidence.manifest?.byteLength, sha256: archiveEvidence.manifest?.sha256 }, "manifest");
