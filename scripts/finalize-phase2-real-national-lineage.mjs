...
const { startedAt, completedAt, elapsedSeconds } = evidence.execution;
const executionCore = { batchId: lineage.batchId, methodVersion: lineage.methodVersion, methodParameterSha256: lineage.methodParameterSha256, inputSetSha256: preflight.sourceVerification.inputSetSha256, startedAt, completedAt, elapsedSeconds, outputCount: lineage.outputs.length, totalRasterBytes: lineage.outputs.reduce((sum, row) => sum + row.byteLength, 0), productionEligible: false };
...
Object.assign(lineage, {
  ...
  execution: { ...executionCore, executionEvidenceCoreSha256: digest(executionCore), requestedConcurrency: 1, observedConcurrencyUpperBound: 1, vCpuCap: 8, ramCapBytes: 17179869184, memoryPeakMeasured: false, memoryLimitEvidence: "Windowed 2048x2048 arrays and synchronous single-child launch; peak RSS was not instrumented.", diskBoundBytes: 237820018840, diskCapBytes: 2199023255552 },
  limitations: ["No patch vectorization or normalized events.", "No boundary intersections or aggregates.", "No tiles, samples, or output statistics."]
});
await writeFile(lineageFile, `${JSON.stringify(lineage, null, 2)}\n`);
console.log(JSON.stringify({lineageFile, executionEvidenceCoreSha256: lineage.execution.executionEvidenceCoreSha256, elapsedSeconds}));
