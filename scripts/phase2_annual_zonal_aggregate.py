...
def empty_stats() -> dict[str, int]:
    return {"known": 0, "loss": 0, "unknown": 0, "outside": 0}
...
def make_row(
    ...
    *,
    ...
    return {
        ...
        "coverageGrade": "complete" if stats["unknown"] == 0 else "partial-with-unknown",
                ...
                for row in range(0, height, ROW_WINDOW):
                    for column in range(0, width, COLUMN_WINDOW):
                        ...
                        for index, (forest_band, loss_band, pair) in enumerate(zip(forest_bands, loss_bands, pairs)):
                            ...
                            baseline_stats["unknown"] += int(np.count_nonzero(selected_forest == NODATA)) if index == 0 else 0
                            stats = stats_by_pair[index]
                            stats["outside"] += int(np.count_nonzero((selected_forest == 0) & (selected_loss == 1)))
                            stats["unknown"] += int(np.count_nonzero((selected_forest == NODATA) | (selected_loss == NODATA)))
                            eligible = (selected_forest == 1) & (selected_loss != NODATA)
