...
export function validatePhase1PermissionOutreachPackage(pkg, matrix) {
  ...
  if (pkg.replyAuditFile !== "data/phase1-outreach-reply-audit.json") throw new Error("Outreach package must point to the bounded reply audit.");
  ...
  for (const message of pkg.messages) {
    ...
    else if (message.status === "already-sent-awaiting-response-no-new-message" || message.status === "already-sent-reply-received-awaiting-resolution") existing += 1;
...
export async function checkPhase1PermissionOutreachPackage() {
  ...
  const [pkg, matrix, replyAudit] = await Promise.all([
  ...
  validatePhase1OutreachReplyAudit(replyAudit, matrix, pkg);
  return validatePhase1PermissionOutreachPackage(pkg, matrix);
}

if (process.argv[1] === fileURLToPath(import.meta.url)) {
  const pkg = await checkPhase1PermissionOutreachPackage();
  console.log(`Phase 1 outreach package passed: ${pkg.messages.filter((message) => typeof message.verifiedSentAt === "string").length} verified sends, one existing request, and bounded reply evidence.`);
}
