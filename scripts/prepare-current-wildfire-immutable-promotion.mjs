...
export function validateCurrentWildfirePromotionPreparation(plan, staged = read("data/staged-acquisitions.json")) {
  ...
  assert.deepEqual(plan.destination, {bucket: "witness-tree-raw-archive-ca-central-1", region: "ca-central-1", countryCode: "CA"}); assert.equal(plan.mfaGatedExecution.required, true); assert.equal(plan.mfaGatedExecution.proposedRole, "WitnessTreeCurrentWildfirePromotionUploader"); assert.equal(plan.mfaGatedExecution.recommendedRetainUntil, "2033-08-12T00:00:00Z"); assert.match(plan.requiredApproval, /four exact payloads.*four deterministic sidecars.*COMPLIANCE.*direct-PutObject\/read-back/i); assert.doesNotMatch(plan.requiredApproval, /GetObjectVersion/); assert.equal(plan.pendingAdditionalIamApproval, "data/current-wildfire-get-object-version-owner-approval.json");
  const expectedGeometry = {"cwfis-current": "profiled-clean", "bc-wildfire": "owner-approved-derived-216-one-permanent-quarantine", "ab-wildfire": "profiled-clean", "on-fire-disturbance": "owner-approved-derived-188-zero-exclusion"}; const wanted = Object.keys(expectedGeometry); assert.equal(plan.artifacts.length, 4); const seen = new Set();
