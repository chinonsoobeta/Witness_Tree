...
Each refusal is asserted in `tests/national-baseline-admission.test.ts`: a non-VLCE2 source, a promotion that is not remote-verified, an off-grid header, a year whose archive filename belongs to another year, a sidecar from another year, and a second snapshot for a year already admitted. The missing-payload-version-ID throw is not reachable through this module, because the immutable-promotion contract already refuses a remote-verified manifest that has no provider version IDs; it narrows an optional type rather than guarding real input.
