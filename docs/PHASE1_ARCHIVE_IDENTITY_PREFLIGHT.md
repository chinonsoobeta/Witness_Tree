...
The helper uses the uploader role for one tiny dedicated exercise upload and a version-specific delete that must be denied. It uses the break-glass role only to set compliance retention and legal-hold status. The MFA-trusted, read-only `WitnessTreeArchiveVerifier` role has `s3:ListBucketVersions` only on the primary bucket and only under `raw/legal-hold-exercises/*`; it has `s3:GetObject`, legal-hold, and retention reads only under that prefix on the primary and recovery buckets. `HeadObject` is authorized by IAM as `s3:GetObject`, not a nonexistent `s3:HeadObject` action. The verifier performs the legal-hold, CloudTrail-status, and recovery-replica readbacks. A failed verifier readback is a recorded verification blocker—not permission to use root or broaden the roles.
