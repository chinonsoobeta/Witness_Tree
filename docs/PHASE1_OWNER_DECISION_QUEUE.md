...
| 6 | Current-wildfire archive gate | `cwfis-current`, `bc-wildfire`, `ab-wildfire`, `on-fire-disturbance` | The existing owner decisions are conditional. All six objects have placeholder-only attestations, but 0/6 have repository-retained concrete version/checksum bindings. Exact-version immutable proof must precede downstream activation. |
