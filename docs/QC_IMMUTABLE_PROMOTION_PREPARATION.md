...
The owner reruns the same approved command with fresh MFA. For saved upload state, the runner requires the recorded sidecar version and classifies `ListParts` before any sidecar write path. Only a single canonical AWS `ListParts` error line with code `(NoSuchUpload)` activates recovery; mixed `AccessDenied`, proxy, multiline or otherwise ambiguous errors stop without a storage write. Recovery never uploads or overwrites a sidecar.
