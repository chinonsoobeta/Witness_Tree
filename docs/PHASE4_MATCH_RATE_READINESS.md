...
* **Granularity.** One run, not per province and not per year. `scope` must be
  ...
  (`scripts/check-phase4-exit-status.mjs:118-123`), and the same object must
...
`["BC", "QC"]` at `scripts/check-phase4-exit-status.mjs:118-123`.
...
1. **[R] Obtain British Columbia forest-event data at all.** Every BC source is
   ...
   `scripts/check-phase4-exit-status.mjs:118-123` requires the scope to be
