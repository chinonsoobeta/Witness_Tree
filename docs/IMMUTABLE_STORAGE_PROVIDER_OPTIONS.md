...
**Status:** Recommendation only. The recommended AWS option has since been provisioned as a bucket in `ca-central-1` with versioning and Object Lock enabled, and three verified staged archives have been uploaded to it and locked per object version under compliance-mode retention until 2033-08-12. The write-once requirement below is now satisfied by demonstration against one of those locked versions; the evidence is in [Demonstrated write-once behaviour](#demonstrated-write-once-behaviour). No default retention policy and no replication configuration exist. Its read-back configuration is recorded in `docs/IMMUTABLE_STORAGE_PROVISIONING.md`. No Azure container or Google Cloud bucket has been created.
...
5. Whether the proof uses only a harmless test object or is authorized to promote either verified raw source. Source attribution/release gates remain independent; Alberta is admitted to transformation design only, and the three snapshots that are now remotely verified are verified as stored bytes only, which admits none of them to transformation, ingestion, or release.

## Exact proof-of-concept sequence after approval

This sequence is intentionally provider-specific only after the owner selects AWS. Step 2 has been carried out in part: the bucket exists with Object Lock and versioning enabled, public access blocked, and SSE-S3 selected, but with no dedicated uploader, reader, or break-glass roles. The sequence below was not the path taken. What actually happened is recorded in [`data/immutable-promotions.json`](../data/immutable-promotions.json): the three verified source archives themselves were uploaded, not a harmless test object, and compliance-mode retention was applied per object version rather than as the bucket default of step 3. The delete and retention-shortening tests of step 3 were run afterwards, against a retained source payload version rather than a test object, and both were refused; see [Demonstrated write-once behaviour](#demonstrated-write-once-behaviour). Steps 5 and 6 have not run.
...
For Azure, the equivalent proof must use a dedicated Canada Central container with a **locked** container-level time-based WORM policy; for GCP, a dedicated Montréal or Toronto regional bucket with the approved retention policy **locked** using Bucket Lock. In all cases, prove an attempted delete/overwrite is rejected and verify remote bytes independently before the project may use `remote-verified`. For the AWS bucket in use, that proof exists and is recorded below. It has not been produced for any other provider, because no Azure container and no Google Cloud bucket has been created.

## Demonstrated write-once behaviour

This section records a demonstration that already happened. It is not an instruction to repeat it.

On 2026-08-13 the auditor ran three adversarial attempts against one locked payload object version in the `ca-central-1` bucket, the Québec historical-fire payload. The attempts used the bucket-owning account with full administrative privileges, which is the most privileged principal available, so a refusal cannot be explained by a narrow uploader policy.

| Attempt | Result | Provider message |
| --- | --- | --- |
| Delete that specific object version | `AccessDenied` | Access Denied because object protected by object lock. |
| Shorten retention from 2033-08-12 to 2027-01-01, keeping compliance mode | `AccessDenied` | Access Denied because object protected by object lock. |
| Downgrade the mode from COMPLIANCE to GOVERNANCE, keeping the same date | `AccessDenied` | Access Denied because object protected by object lock. |

A post-check after all three attempts read the object version back unchanged: byte length 414,244,435, the recorded CRC64NVME, `ObjectLockMode` COMPLIANCE, `ObjectLockRetainUntilDate` 2033-08-12T00:00:00+00:00, and no delete markers under the prefix. The full record, including the target version and the exact values, is in [`data/immutable-promotions.json`](../data/immutable-promotions.json) under `writeOnceDemonstration`.

**Scope of this evidence.** The demonstration was run against one of the three locked payload objects, not all three. All three carry the same compliance mode and the same 2033-08-12 retention date, set the same way. A compliance-mode retention cannot be shortened or removed by anyone, including the account root, until it expires. The demonstration covers deletion and retention weakening. It does not cover every conceivable loss mode. Bucket deletion, account closure, and region loss are separate risks and remain out of scope for this evidence.

## Recorded version IDs were resolved against S3

On 2026-08-13 the auditor resolved all six recorded version IDs, three payload and three manifest, against live S3. Every one exists. Each payload version returned the recorded byte length, the recorded CRC64NVME, and compliance retention until 2033-08-12. Each manifest version returned the recorded byte length and no lock, which matches the recorded `not-locked` state.

That check is point-in-time and CI does not repeat it. CI cannot repeat it: resolving a version ID requires a call to S3 and CI holds no S3 credentials. The consequence is a real gap. A version ID is the only field that pins the record to one specific immutable object version, so a wrong value would leave the record pointing at nothing while every automated check still passed. The auditor confirmed this by mutation: changing a recorded payload version ID to a different well-formed value fails zero tests. Two other mutations were run for contrast and both were caught, so the suite is not blind in general.

`tests/archive-staging-promotions.test.ts` now asserts the shape of each recorded version ID, so a truncated, empty, placeholder, or obviously malformed value fails. Shape is not identity. A different but well-formed version ID still passes, so that test narrows the gap without closing it.
