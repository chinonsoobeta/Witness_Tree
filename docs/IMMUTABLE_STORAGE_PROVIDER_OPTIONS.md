...
**Status:** Recommendation only. The recommended AWS option has since been provisioned as a bucket in `ca-central-1` with versioning and Object Lock enabled, and three verified staged archives have been uploaded to it and locked per object version under compliance-mode retention until 2033-08-12. No default retention policy and no replication configuration exist. Its read-back configuration is recorded in `docs/IMMUTABLE_STORAGE_PROVISIONING.md`. No Azure container or Google Cloud bucket has been created.
...
This sequence is intentionally provider-specific only after the owner selects AWS. Step 2 has been carried out in part: the bucket exists with Object Lock and versioning enabled, public access blocked, and SSE-S3 selected, but with no dedicated uploader, reader, or break-glass roles. The sequence below was not the path taken. What actually happened is recorded in [`data/immutable-promotions.json`](../data/immutable-promotions.json): the three verified source archives themselves were uploaded, not a harmless test object, and compliance-mode retention was applied per object version rather than as the bucket default of step 3. No delete attempt and no retention-shortening attempt against a retained version is recorded, so the write-once behaviour of this bucket has not been demonstrated by test. Steps 5 and 6 have not run.
