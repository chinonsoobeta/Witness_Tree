...
| ---: | --- | --- | --- | --- | ---: |
| 1 | Canopy owner decision and federal-only promotion | Archived canopy height, plus one local federal ZIP shared by two rows | Canopy may proceed only to its named owner source-ledger decision; the federal no-write preflight remains available. | Do not revisit canopy storage. Separately approve a federal-only path. | +0.50 raw; +0.483871 percentage points |
...
- The four current-wildfire rows now pass the **6/6** primary immutable-object gate. A local preflight remains available, but derived recovery/mutation provenance and separate transformation, ingestion, release and production-admission records are missing. Raw score impact is zero because the four rows already have raw credit.
...
The later canopy recovery and current-wildfire primary readback records supplied redacted exact-version and retention evidence without retaining provider identifiers. The canonical baseline is **15.25/31 (39.7580645%)**, with **11 remote / 5 local / 2 partial / 13 access-blocked** rows, **11 immutable rows**, **0/31 production admission**, and a **6/6 current-wildfire primary gate**; recovery/mutation provenance and downstream admission remain blocked.
