...
| ---: | --- | --- | --- | --- | ---: |
| 1 | NBAC recovery and archive-control completion | `cwfis-historical`; one 1,257,052,370-byte ZIP plus its manifest | The current official OGL Canada metadata, local checksum/profile, exact-key IAM evidence, and durable primary-readback/COMPLIANCE receipt are bound. Recovery and universal archive claims remain false. | Owner-controlled recovery-replica and archive-control evidence must be captured for the exact payload and manifest, then transformation, ingestion, release, publication, and admission remain separate decisions. | +0 raw; +0 points |
...
- NBAC: the exact ZIP is checksum-bound and profiled with passed ZIP integrity, 52,610 polygons, and 49 quarantined ring self-intersections. The durable receipt binds exact-version primary payload/manifest readback and COMPLIANCE retention; recovery, transformation, ingestion, release, publication, and production admission remain false.
...
The canonical baseline is **17.00/31 (41.4516129%)**, with **14 remote-verified-archived-profiled, 1 local-verified-profiled (`cwfis-historical`), 1 partial-component, 13 access-blocked, and 2 production-admitted** rows, **16 immutable rows**, **2/31 production admission**, and a **6/6 primary current-wildfire archive gate**. Québec’s 62 exact objects have independent redacted readback; this remains raw archive evidence only. NBAC has a durable exact-version primary-readback receipt, but recovery and universal archive proof remain separate. The formal Phase 1 exit is **2/4 (50%)**, core production admission is **2/22**, recovery proof is separate, and remaining downstream admission is blocked.
