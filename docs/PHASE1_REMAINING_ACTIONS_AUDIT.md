...
- The four current-wildfire rows pass the **6/6** immutable-object condition and are production admitted under the recorded downstream reconciliation. They have no remaining admission action. Raw score impact is zero because the four rows already had full raw credit.
...
The canonical baseline is **15.25/31 (39.7580645%)**, with **11 remote / 5 local / 2 partial / 13 access-blocked** rows and **11 immutable rows**. The current-wildfire gate is **6/6**, and its four rows are now production admitted. The remaining-action selection therefore covers the other **27/31** rows; the raw-evidence score is unchanged.
