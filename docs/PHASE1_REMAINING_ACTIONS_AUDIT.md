...
| ---: | --- | --- | --- | --- | ---: |
| 1 | Federal-only archive promotion | One local federal ZIP shared by two rows | Exact approval and no-write preflight are recorded; the runner now requires a separate live IAM attestation and primary-only boundary. | Owner-local `--run-federal` with fresh MFA, then exact primary readback; recovery authorization/proof remains a separate required gate before any credit claim. | +0.50 raw; +0.483871 percentage points |
