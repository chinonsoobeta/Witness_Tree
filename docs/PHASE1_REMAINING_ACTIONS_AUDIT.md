...
| ---: | --- | --- | --- | --- | ---: |
| 1 | National local archive preflight and owner promotion | Canopy height, plus one federal ZIP shared by the two Elections Canada rows; **2 artifacts / 3 rows** | Delegate may run the no-write local preflight. | Fresh owner approval, a safe canopy/federal-only path that does not revisit harvest, MFA, and exact primary/recovery readbacks. | +0.75 raw; +0.7258065 percentage points |
...
- The two partial rows remain at 0.25 raw credit each. The historical row needs owner review of the exact NBAC agreement and written consent plus the missing artifact. The boundary row needs owner review of the Alberta and Québec requests, written permission, and a stable current Québec 2017 artifact. Neither unsent request is a permission or acquisition.
...
No AWS, email, form, archive mutation, permission grant, Phase 2 action, push, or deployment was performed for this audit. The owner-only canopy resume state passed offline mode-600 validation in the existing evidence, but no private path, contents, upload identifier, provider part identifiers, exact-version readback, or COMPLIANCE-retention readback is copied into this audit.
