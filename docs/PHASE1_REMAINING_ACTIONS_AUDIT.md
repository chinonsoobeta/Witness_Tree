...
| ---: | --- | --- | --- | --- | ---: |
| 1 | Federal-only archive promotion | One local federal ZIP shared by two rows | Exact approval and local-only preflight are recorded; canonical readiness remains blocked, raw live-IAM file evidence is absent, and external execution is intentionally not implemented. | Separately review canonical owner-readiness evidence, exact finite raw IAM evidence, and a future descriptor-bound execution implementation; recovery authorization/proof remains a distinct gate before any credit claim. | +0.50 raw; +0.483871 percentage points |
