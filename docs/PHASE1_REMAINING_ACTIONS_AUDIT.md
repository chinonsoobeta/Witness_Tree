...
- National: `zsh scripts/run-phase1-approved-promotion.sh --preflight` passed all three bound artifacts (harvest revalidated at 247,945,479 bytes, canopy at 10,347,564,066 bytes, and federal at 10,301,648 bytes; total 10,605,811,193 bytes). The harvest remains excluded from any new promotion. Canopy and federal remain local-only. Planned sidecar keys and the federal-only boundary remain those in the runner and [`data/phase1-local-profiled-promotion-preparation.json`](../data/phase1-local-profiled-promotion-preparation.json).
...
No AWS, email, form, archive mutation, permission grant, Phase 2 action, push, or deployment was performed for this audit. The latest redacted national record confirms canopy MPU completion and exact-byte/`FULL_OBJECT` payload and sidecar heads in primary and recovery, but no private path, contents, upload identifier, provider part identifiers, exact-version readback, or COMPLIANCE-retention readback is copied into this audit; no immutable archive credit is claimed. The fresh local receipts likewise do not change the **15/31 (39.516129%)** baseline, the **10 remote / 6 local / 2 partial / 13 access-blocked** counts, **10 immutable rows**, **0/31 production admission**, or **4/6 current-wildfire** gate.
