...
The `raw/qc-fourth-inventory/` prefix is empty on both buckets. `node scripts/qc-fourth-inventory-immutable-promotion.mjs` is dry-run only. The execute form remains blocked until all four independent approvals are true: exact artifact set, least-privilege IAM policy, irreversible COMPLIANCE retention, and MFA session. The command template uses owner-supplied placeholders and was not run.
