...
The local Québec preflight remains safe. The owner explicitly approved the additional exact IAM block in `docs/QC_IMMUTABLE_PROMOTION_PREPARATION.md`, including four-resource `s3:GetObjectVersion`. Access Analyzer returned zero findings and exact positive/negative custom-policy simulations passed. A live read-only audit found that the exact role and dedicated operator policy do not exist. Privileged provisioning was not available in this delegated task, so `--run` remains blocked until root-side provisioning and exact readback pass. No IAM or S3 mutation occurred.
