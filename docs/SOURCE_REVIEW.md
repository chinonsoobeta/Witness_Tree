...
"Accepted", in this document, means only what the staging evidence in [`docs/SOURCE_VERIFICATION.md`](SOURCE_VERIFICATION.md) and [`data/staged-acquisitions.json`](../data/staged-acquisitions.json) already records: admission to verified local staging and transformation design for Québec and Alberta. It does not mean ingestion or production eligibility. The separately verified immutable-storage state is read from [`data/immutable-promotions.json`](../data/immutable-promotions.json); recording it here neither grants nor changes it.
...
`data/source-review-register.json` holds the accepted baselines a future review would compare against, and an empty `reviews` list. Each baseline is checked against its staged acquisition record, and its observed immutable-promotion state is checked against the separate promotion record, so a baseline cannot state a checksum, byte length, acceptance time, attribution, or immutable-storage state that those records do not carry. Where a baseline lacks evidence it says so:
...
- **Alberta AVI Crown** — checksum and Open Government Licence – Alberta version 2.2 attribution baseline recorded; no publisher version identifier is recorded, so an update cannot be distinguished from a correction and `updateVersusCorrection` stays `undetermined`.

Neither source has been ingested or connected to product records. Both have separately evidenced `remote-verified` immutable payload snapshots; that storage fact does not make either source production eligible.
