...
4. Create a source-review process for updates, corrections, licence changes, and reprocessing. Do not fabricate a successful acquisition or current coverage. The process, its evidence requirements, and its acceptance states are defined in [`docs/SOURCE_REVIEW.md`](SOURCE_REVIEW.md), with the decision contract in `lib/source-review/` and the registered baselines in `data/source-review-register.json`. No review has been run against a live publisher change; the register records baselines only. It records separately verified immutable-storage state but cannot grant re-acceptance, immutable promotion, or production eligibility.
