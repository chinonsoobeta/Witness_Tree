...
**Status:** Proposed implementation design. One empty AWS S3 bucket now exists in `ca-central-1` with versioning and Object Lock enabled, SSE-S3 default encryption, and block-all-public-access on. No default retention is set, deliberately, because compliance-mode retention cannot be shortened by anyone including the account root, so the duration is the owner's decision. Three verified staged archives have since been uploaded to canonical snapshot keys in that bucket and locked under compliance-mode retention until 2033-08-12; their evidence is in [`data/immutable-promotions.json`](../data/immutable-promotions.json). No replication target exists, no production archive exists, and the required controls below are still only partly satisfied, as mapped row by row in `docs/IMMUTABLE_STORAGE_PROVISIONING.md`. The read-back configuration is recorded in `docs/IMMUTABLE_STORAGE_PROVISIONING.md`.
