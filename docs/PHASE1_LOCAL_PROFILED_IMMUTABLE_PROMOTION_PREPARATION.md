...
Run `npm run prepare:phase1-local-profiled-immutable-promotion` to validate local bindings and deterministic payload/sidecar keys. It does not contain AWS code or credentials and cannot upload, create a bucket, set Object Lock, alter retention, transform, ingest, or release data.
