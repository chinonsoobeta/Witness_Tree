...
`--run` sends `If-None-Match: *` for every payload and sidecar and reads each object back with the returned version ID. A pre-existing exact key or a race therefore stops the run without an overwrite. The raw runner has no resumable checkpoint: if an interruption leaves earlier objects written, preserve the owner-local evidence and obtain a fresh owner decision before any retry; do not infer completion or rerun blindly. The runner still does not create a recovery replica or update the canonical redacted evidence record. Those remain separate owner-authorized evidence steps.
